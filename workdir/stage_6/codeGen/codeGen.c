...
RegIndex codeGenAssgn(ASTNode* node, FILE* fp){
    ...
    else if(id->nodetype == NODE_FIELD){
        int addr = getFieldAddr(id, fp);
        fprintf(fp, "MOV [R%d], R%d\n", addr, expr);
...
int codeGenNODE(ASTNode* node, FILE* fp){
    switch(node->nodetype){
        ...
        case NODE_NULL:
            return codeGenNull(node, fp);

        case NODE_FREE:
            return codeGenFree(node, fp);

        case NODE_INIT:
            return codeGenInit(node, fp);

        case NODE_ALLOC:
            return codeGenAlloc(node, fp);

        case NODE_FIELD:
            return codeGenField(node, fp);
