...
ASTNode* makeAssgnNode(ASTNode* l, ASTNode* r){
    if((l->nodetype != NODE_LEAF && l->nodetype != NODE_ARRAY && l->nodetype != NODE_PTR && l->nodetype != NODE_FIELD) || (l->nodetype != NODE_PTR && l->name == NULL)){
...
ASTNode* makeInitNode(void){
    ASTNode* temp = TreeCreate(NULL, TLookup("integer"), NULL, NODE_INIT, NULL, NULL, NULL, NULL); 
    return temp;
}

ASTNode* makeAllocNode(void){
    ASTNode* temp = TreeCreate(NULL, TLookup("void"), NULL, NODE_ALLOC, NULL, NULL, NULL, NULL); 
    return temp;
}

ASTNode* makeFreeNode(ASTNode* freenode){
    ASTNode* temp = TreeCreate(NULL, TLookup("void"), NULL, NODE_FREE, freenode, NULL, NULL, NULL); 
    return temp;
}

ASTNode* makeFieldNode(ASTNode* head, char* fieldname){
    FieldList* field = FLookup(fieldname, head->type->fields);

    if(field == NULL){
        fprintf(stderr, "%s does not have a field %s\n", head->name, fieldname);
        exit(1);
    }

    ASTNode* fieldnode = TreeCreate(NULL, field->type, fieldname, NODE_LEAF, NULL, NULL, NULL, NULL); 

    ASTNode* temp = TreeCreate(NULL, field->type, fieldname, NODE_FIELD, head, NULL, fieldnode, NULL);
    return temp;
}

ASTNode* makeNullNode(void){
    ASTNode* temp = TreeCreate(NULL, TLookup("void"), NULL, NODE_NULL, NULL, NULL, NULL, NULL); 
    return temp;
}